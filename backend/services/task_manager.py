...
def save_image_with_version(image, project_id: str, page_id: str, file_service,
	...
	max_version = db.query(func.max(PageImageVersion.version_number)).filter_by(page_id=page_id).scalar() or 0
...
def generate_descriptions_task(task_id: str, project_id: str, ai_service,
	...
	if app is None:
		...
		# 在FastAPI中不需要app_context()，直接执行任务逻辑
		try:
